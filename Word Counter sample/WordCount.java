...
{/* Extend the custom class with Mapper<IpKey, IpValue, OpKey, OpValue >IpKey ->InputKey
     to mapper class (Object in most cases)IpValue ->InputValue to mapper class
      (Text in most cases)OpKey ->OutputKey from mapper class
       ...
       public static class TokenizerMapper extends Mapper<Object, Text, Text, IntWritable>
       {
        ...
        /* Extend the custom class with Reducer<IpKey, IpValue, OpKey, OpValue >IpKey ->InputKey
         to reducer class (OutputKey from mapper class)IpValue ->InputValue to
          reducer class (OuputValue from mapper class)OpKey ->OutputKey from reducer class 
          (Will be written to file)OpValue ->OutputValue from reducer class(Will bewritten to file)*/
            ...
            public static void main(String[] args) throws Exception 
            {
                ...
                /* Create an object of org.apache.hadoop.mapreduce.Job.Creates a new Jobwith a given jobName.
                 The Job makes a copy of theConfiguration so that any necessary internal modifications do not
                  ...
                  /* The following command is used to run a mapreduce program.
                  hadoop jar wc.jar WordCount input_dir output_dirhadoop jar
                   <jar name><pgm class name><input path><output path>
                   The input and output path to the dataset are set as below*/
                   FileInputFormat.addInputPath(job, new Path(args[0]));
                   FileOutputFormat.setOutputPath(job, new Path(args[1]));
                   /* Wait for the program execution to complete.Exit the program after job execution.*/
